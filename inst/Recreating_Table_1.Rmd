...
for(i in 1:10){
  t2<- system.time({
  sqlitemodel<-dbglm(isred~power_rating+number_of_seats+gross_vehicle_mass,tbl=cars1)
  })
  t2<- unname(t2[3])
  a1<- c(a1, t2)
  t6<- mean(a1)
}
duckmodel<-dbglm(isred~power_rating+number_of_seats+gross_vehicle_mass,tbl=cars1)
row.11<- c(unname(duckmodel$hatbeta[,1]),t6)
row.12<- unname(sqrt(diag(duckmodel$hatV)))
...
duckmodel<-dbglm(isred~power_rating+number_of_seats+gross_vehicle_mass,tbl=cars1)
row.13<- c(unname(duckmodel$hatbeta[,1]),t7)
row.14<- unname(sqrt(diag(duckmodel$hatV)))
